...
install_linaro_cross_compiler() {
	...
	TOOLCHAIN_VERSION="7.3-2018.05"
	if [ "${TOOLCHAIN_VERSION}" = "6.4-2018.05" ]; then
		if [ "${OS_32BIT_64BIT}" = "x86_64" ]; then
			...
			TOOLCHAIN="gcc-linaro-6.4.1-2018.05-x86_64_arm-linux-gnueabihf"
		else
			# gcc build with: --with-tune=cortex-a9 --with-arch=armv7-a --with-fpu=vfpv3-d16 --with-float=hard --with-mode=thumb
			TOOLCHAIN="gcc-linaro-6.4.1-2018.05-i686_arm-linux-gnueabihf"
		fi
		TOOLCHAIN_DOWNLOAD_URL="https://releases.linaro.org/components/toolchain/binaries/6.4-2018.05/arm-linux-gnueabihf/${TOOLCHAIN}.tar.xz"

	elif [ "${TOOLCHAIN_VERSION}" = "7.3-2018.05" ]; then
		if [ "${OS_32BIT_64BIT}" = "x86_64" ]; then
			# gcc build with: --with-tune=cortex-a9 --with-arch=armv7-a --with-fpu=vfpv3-d16 --with-float=hard --with-mode=thumb
			TOOLCHAIN="gcc-linaro-7.3.1-2018.05-x86_64_arm-linux-gnueabihf"
		else
			# gcc build with: --with-tune=cortex-a9 --with-arch=armv7-a --with-fpu=vfpv3-d16 --with-float=hard --with-mode=thumb
			TOOLCHAIN="gcc-linaro-7.3.1-2018.05-i686_arm-linux-gnueabihf"
		fi
		TOOLCHAIN_DOWNLOAD_URL="https://releases.linaro.org/components/toolchain/binaries/7.3-2018.05/arm-linux-gnueabihf/${TOOLCHAIN}.tar.xz"
	fi
